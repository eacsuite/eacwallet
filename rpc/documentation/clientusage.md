...
import (
	...
	pb "github.com/eacsuite/eacwallet/rpc/walletrpc"
...
var certificateFile = filepath.Join(eacutil.AppDataDir("eacwallet", false), "rpc.cert")
...
func main() {
	...
	fmt.Println("Spendable balance: ", eacutil.Amount(balanceResponse.Spendable))
}
```
...
```bash
$ protoc -I/path/to/eacwallet/rpc --cpp_out=. --grpc_out=. \
  --plugin=protoc-gen-grpc=$(which grpc_cpp_plugin) \
  /path/to/eacwallet/rpc/api.proto
```
...
auto main() -> int {
    ...
    // configured to use ECDSA TLS certificates (required by eacwallet).
    ...
    auto wallet_tls_cert_file = []{
        ...
        return pw->pw_dir + "/.eacwallet/rpc.cert"s;
    }();
...
```PowerShell
PS> & protoc.exe -I \Path\To\eacwallet\rpc --csharp_out=Example --grpc_out=Example `
...
namespace Example
{
    static class Program
    {
        ...
        static async Task ExampleAsync()
        {
            ...
            // certificates (required by eacwallet).
            Environment.SetEnvironmentVariable("GRPC_SSL_CIPHER_SUITES", "HIGH+ECDSA");

            var walletAppData = Portability.LocalAppData(Environment.OSVersion.Platform, "Eacwallet");
...
var certPath = path.join(process.env.HOME, '.eacwallet', 'rpc.cert');
if (os.platform == 'win32') {
    certPath = path.join(process.env.LOCALAPPDATA, 'Eacwallet', 'rpc.cert');
...
```bash
$ protoc -I /path/to/eacsuite/eacwallet/rpc --python_out=. --grpc_out=. \
...
def main():
    ...
    cert_file_path = os.path.join(os.environ['HOME'], '.eacwallet', 'rpc.cert')
    if platform.system() == 'Windows':
        cert_file_path = os.path.join(os.environ['LOCALAPPDATA'], "Eacwallet", "rpc.cert")
