...
import (
	...
	"github.com/eacsuite/eacwallet/wallet"
...
func makeMultiSigScript(w *wallet.Wallet, keys []string, nRequired int) ([]byte, error) {
	keysesPrecious := make([]*eacutil.AddressPubKey, len(keys))
	...
	for i, a := range keys {
		...
		default:
			...
			pubKeyAddr, err := eacutil.NewAddressPubKey(
...
func addMultiSigAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	secp256k1Addrs := make([]eacutil.Address, len(cmd.Keys))
...
func createMultiSig(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	address, err := eacutil.NewAddressScriptHash(script, w.ChainParams())
	if err != nil {
...
func decodeAddress(s string, params *chaincfg.Params) (eacutil.Address, error) {
	addr, err := eacutil.DecodeAddress(s, params)
	if err != nil {
...
func importPrivKey(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	wif, err := eacutil.DecodeWIF(cmd.PrivKey)
	if err != nil {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCInvalidAddressOrKey,
...
func getTransaction(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	var (
		debitTotal  eacutil.Amount
		creditTotal eacutil.Amount // Excludes change
		fee         eacutil.Amount
		feeF64      float64
	)
...
// and amounts.
func makeOutputs(pairs map[string]eacutil.Amount, chainParams *chaincfg.Params) ([]*wire.TxOut, error) {
	outputs := make([]*wire.TxOut, 0, len(pairs))
	for addrStr, amt := range pairs {
		addr, err := eacutil.DecodeAddress(addrStr, chainParams)
		if err != nil {
...
func sendPairs(w *wallet.Wallet, amounts map[string]eacutil.Amount,
	account uint32, minconf int32, feeSatPerKb eacutil.Amount) (string, error) {
...
func sendFrom(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	amt, err := eacutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, err
	}
	pairs := map[string]eacutil.Amount{
		cmd.ToAddress: amt,
	}
...
func sendMany(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	pairs := make(map[string]eacutil.Amount, len(cmd.Amounts))
	for k, v := range cmd.Amounts {
		amt, err := eacutil.NewAmount(v)
		if err != nil {
			return nil, err
		}
		pairs[k] = amt
...
func signRawTransaction(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	for _, rti := range cmdInputs {
		...
		if cmd.PrivKeys != nil && len(*cmd.PrivKeys) != 0 {
			...
			addr, err := eacutil.NewAddressScriptHash(redeemScript,
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
	...
	var keys map[string]*eacutil.WIF
	if cmd.PrivKeys != nil {
		keys = make(map[string]*eacutil.WIF)

		for _, key := range *cmd.PrivKeys {
			wif, err := eacutil.DecodeWIF(key)
			if err != nil {
				return nil, DeserializationError{err}
			}
			...
			addr, err := eacutil.NewAddressPubKey(wif.SerializePubKey(),
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
...
func verifyMessage(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	case *eacutil.AddressPubKeyHash: // ok
		return bytes.Equal(eacutil.Hash160(serializedPubKey), checkAddr.Hash160()[:]), nil
	case *eacutil.AddressPubKey: // ok
