...
//
//   https://github.com/eacsuite/eacwallet/blob/master/rpc/documentation/api.md
//
// Any API changes must be performed according to the steps listed here:
//
//   https://github.com/eacsuite/eacwallet/blob/master/rpc/documentation/serverchanges.md
package rpcserver
...
import (
	...
	"github.com/eacsuite/eacwallet/internal/cfgutil"
	"github.com/eacsuite/eacwallet/internal/zero"
	"github.com/eacsuite/eacwallet/netparams"
...
// as well as establishing a RPC connection to a eacd consensus server.
...
func (s *walletServer) ImportPrivateKey(ctx context.Context, req *pb.ImportPrivateKeyRequest) (
	*pb.ImportPrivateKeyResponse, error) {
	...
	wif, err := eacutil.DecodeWIF(req.PrivateKeyWif)
	if err != nil {
...
func (s *walletServer) FundTransaction(ctx context.Context, req *pb.FundTransactionRequest) (
	*pb.FundTransactionResponse, error) {
	...
	var totalAmount eacutil.Amount
	for _, output := range unspentOutputs {
		...
		totalAmount += eacutil.Amount(output.Output.Value)

		if req.TargetAmount != 0 && totalAmount > eacutil.Amount(req.TargetAmount) {
			break
		}
	}

	var changeScript []byte
	if req.IncludeChangeScript && totalAmount > eacutil.Amount(req.TargetAmount) {
...
//   sendrawtransaction, so connection errors to eacd could result in the tx
