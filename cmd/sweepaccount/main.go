...
var (
	walletDataDirectory = eacutil.AppDataDir("eacwallet", false)
	newlineBytes        = []byte{'\n'}
)
...
// Flags.
var opts = struct {
	TestNet3              bool                `long:"testnet" description:"Use the test earthcoin network (version 4)"`
...
func makeInputSource(outputs []btcjson.ListUnspentResult) txauthor.InputSource {
	var (
		totalInputValue eacutil.Amount
		inputs          = make([]*wire.TxIn, 0, len(outputs))
		inputValues     = make([]eacutil.Amount, 0, len(outputs))
		sourceErr       error
	)
	for _, output := range outputs {
		outputAmount, err := eacutil.NewAmount(output.Amount)
		if err != nil {
	...
	return func(eacutil.Amount) (eacutil.Amount, []*wire.TxIn, []eacutil.Amount, [][]byte, error) {
...
func sweep() error {
	...
	for _, previousOutputs := range sourceOutputs {
		...
		outputAmount := eacutil.Amount(tx.Tx.TxOut[0].Value)
...
func saneOutputValue(amount eacutil.Amount) bool {
	return amount >= 0 && amount <= eacutil.MaxSatoshi
}
