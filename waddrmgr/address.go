...
// Address returns the eacutil.Address which represents the managed address.
// This will be a pay-to-pubkey-hash address.
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() eacutil.Address {
	return a.address
}
...
//
// This is part of the ManagedPubKeyAddress interface implementation.
func (a *managedAddress) ExportPrivKey() (*eacutil.WIF, error) {
	pk, err := a.PrivKey()
	if err != nil {
		return nil, err
	}

	return eacutil.NewWIF(pk, a.manager.rootManager.chainParams, a.compressed)
}
...
func newManagedAddressWithoutPrivKey(m *ScopedKeyManager,
	...
	if compressed {
		...
		pubKeyHash = eacutil.Hash160(pubKey.SerializeUncompressed())
	}

	var address eacutil.Address
	var err error
	...
	case NestedWitnessPubKey:
		...
		witAddr, err := eacutil.NewAddressWitnessPubKeyHash(
		...
		address, err = eacutil.NewAddressScriptHash(
	...
	case WitnessPubKey:
		address, err = eacutil.NewAddressWitnessPubKeyHash(
			pubKeyHash, m.rootManager.chainParams,
		)
		if err != nil {
			return nil, err
...
type scriptAddress struct {
	manager         *ScopedKeyManager
	account         uint32
	address         *eacutil.AddressScriptHash
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() eacutil.Address {
	return a.address
}
...
func newScriptAddress(m *ScopedKeyManager, account uint32, scriptHash,
	...
	address, err := eacutil.NewAddressScriptHashFromHash(
