...
var (
	...
	TstRecvTx, _                    = eacutil.NewTxFromBytes(TstRecvSerializedTx)
	...
	TstSpendingTx, _           = eacutil.NewTxFromBytes(TstSpendingSerializedTx)
...
func serializeTx(tx *eacutil.Tx) []byte {
...
func TestInsertsCreditsDebitsRollbacks(t *testing.T) {
	...
	dupRecvTx, _ := eacutil.NewTxFromBytes(TstRecvSerializedTx)
	...
	TstDoubleSpendTx := eacutil.NewTx(newDupMsgTx)
	...
	TstSpendingTx := eacutil.NewTx(spendingTx)
	...
	tests := []struct {
		...
		},
		{
			...
			bal: 0,
			unc: eacutil.Amount(TstRecvTx.MsgTx().TxOut[0].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstRecvTx.Hash(),
					Index: 0,
		...
		},
		{
			...
			bal: eacutil.Amount(TstRecvTx.MsgTx().TxOut[0].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstRecvTx.Hash(),
		...
		},
		{
			...
			bal: eacutil.Amount(TstDoubleSpendTx.MsgTx().TxOut[0].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
		...
		},
		{
			...
			bal: 0,
			unc: eacutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
		...
		},
		{
			...
			bal: 0,
			unc: eacutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstSpendingTx.Hash(),
					Index: 0,
		...
		},
		{
			...
			bal: eacutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstSpendingTx.Hash(),
...
func TestFindingSpentCredits(t *testing.T) {
	...
	expectedBal := eacutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value)
	if bal != expectedBal {
...
func TestRemoveUnminedTx(t *testing.T) {
	...
	// balance.
	checkBalance := func(expectedBalance eacutil.Amount,
	...
	checkBalance(eacutil.Amount(initialBalance), false)
	checkBalance(eacutil.Amount(initialBalance), true)
	...
	checkBalance(eacutil.Amount(changeAmount), true)
...
func assertBalance(t *testing.T, s *Store, ns walletdb.ReadWriteBucket,
	confirmed bool, blockHeight int32, exp eacutil.Amount) {
...
func TestOutputLocks(t *testing.T) {
	...
	coinbase := newCoinBase(
		eacutil.SatoshiPerBitcoin, eacutil.SatoshiPerBitcoin*2,
	)
	coinbaseHash := coinbase.TxHash()

	// One of the spends will be unconfirmed.
	const unconfirmedBalance = eacutil.SatoshiPerBitcoin / 2
	...
	const confirmedBalance = eacutil.SatoshiPerBitcoin
