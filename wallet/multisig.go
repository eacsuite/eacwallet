...
func (w *Wallet) MakeMultiSigScript(addrs []eacutil.Address, nRequired int) ([]byte, error) {
	pubKeys := make([]*eacutil.AddressPubKey, len(addrs))
...
func (w *Wallet) ImportP2SHRedeemScript(script []byte) (*eacutil.AddressScriptHash, error) {
	var p2shAddr *eacutil.AddressScriptHash
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			...
			if waddrmgr.IsError(err, waddrmgr.ErrDuplicateAddress) {
				...
				p2shAddr, _ = eacutil.NewAddressScriptHash(script,
					w.chainParams)
				return nil
			}
			return err
		...
		p2shAddr = addrInfo.Address().(*eacutil.AddressScriptHash)
		return nil
	})
	return p2shAddr, err
}
