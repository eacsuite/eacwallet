...
import (
	...
	"github.com/eacsuite/eacwallet/walletdb/migration"
...
// rescan request.
func (w *Wallet) activeData(dbtx walletdb.ReadWriteTx) ([]eacutil.Address, []wtxmgr.Credit, error) {
	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
	txmgrNs := dbtx.ReadWriteBucket(wtxmgrNamespaceKey)

	var addrs []eacutil.Address
	err := w.Manager.ForEachRelevantActiveAddress(
		addrmgrNs, func(addr eacutil.Address) error {
...
func newFilterBlocksRequest(batch []wtxmgr.BlockMeta,
	...
	filterReq := &chain.FilterBlocksRequest{
		Blocks:           batch,
		ExternalAddrs:    make(map[waddrmgr.ScopedIndex]eacutil.Address),
		InternalAddrs:    make(map[waddrmgr.ScopedIndex]eacutil.Address),
...
func (w *Wallet) CreateSimpleTx(account uint32, outputs []*wire.TxOut,
	minconf int32, satPerKb eacutil.Amount, dryRun bool) (
...
// account.
func (w *Wallet) AccountAddresses(account uint32) (addrs []eacutil.Address, err error) {
	err = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
// include a UTXO.
func (w *Wallet) CalculateBalance(confirms int32) (eacutil.Amount, error) {
...
type Balances struct {
	Total          eacutil.Amount
	Spendable      eacutil.Amount
	ImmatureReward eacutil.Amount
}
...
func (w *Wallet) CurrentAddress(account uint32, scope waddrmgr.KeyScope) (eacutil.Address, error) {
	chainClient, err := w.requireChainClient()
	if err != nil {
		return nil, err
	}
	...
	if props != nil {
		err = chainClient.NotifyReceived([]eacutil.Address{addr})
		if err != nil {
			return nil, err
		}
...
func (w *Wallet) PubKeyForAddress(a eacutil.Address) (*btcec.PublicKey, error) {
...
// address.
func (w *Wallet) PrivKeyForAddress(a eacutil.Address) (*btcec.PrivateKey, error) {
...
func (w *Wallet) HaveAddress(a eacutil.Address) (bool, error) {
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
func (w *Wallet) AccountOfAddress(a eacutil.Address) (uint32, error) {
	var account uint32
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		var err error
...
func (w *Wallet) AddressInfo(a eacutil.Address) (waddrmgr.ManagedAddress, error) {
...
func listTransactions(tx walletdb.ReadTx, details *wtxmgr.TxDetails, addrMgr *waddrmgr.Manager,
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		var debitTotal eacutil.Amount
		for _, deb := range details.Debits {
			debitTotal += deb.Amount
		}
		var outputTotal eacutil.Amount
		for _, output := range details.MsgTx.TxOut {
			outputTotal += eacutil.Amount(output.Value)
		}
...
outputs:
	for i, output := range details.MsgTx.TxOut {
		...
		amountF64 := eacutil.Amount(output.Value).ToBTC()
		...
		loopDetails:
			for i := range details {
				...
				for _, cred := range detail.Credits {
					...
					apkh, ok := addrs[0].(*eacutil.AddressPubKeyHash)
					if !ok {
						continue
					}
...
func (w *Wallet) Accounts(scope waddrmgr.KeyScope) (*AccountsResult, error) {
	...
	err = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		m := make(map[uint32]*eacutil.Amount)
...
func (w *Wallet) DumpPrivKeys() ([]string, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		// privkeys.
		return w.Manager.ForEachActiveAddress(addrmgrNs, func(addr eacutil.Address) error {
...
func (w *Wallet) DumpWIFPrivateKey(addr eacutil.Address) (string, error) {
...
func (w *Wallet) ImportPrivateKey(scope waddrmgr.KeyScope, wif *eacutil.WIF,
	...
	if rescan {
		...
		err := w.chainClient.NotifyReceived([]eacutil.Address{addr})
		if err != nil {
...
func (w *Wallet) NewAddress(account uint32,
	scope waddrmgr.KeyScope) (eacutil.Address, error) {
...
func (w *Wallet) newAddress(addrmgrNs walletdb.ReadWriteBucket, account uint32,
	scope waddrmgr.KeyScope) (eacutil.Address, *waddrmgr.AccountProperties, error) {
...
func (w *Wallet) newChangeAddress(addrmgrNs walletdb.ReadWriteBucket,
	account uint32, scope waddrmgr.KeyScope) (eacutil.Address, error) {
...
// address.
func (w *Wallet) TotalReceivedForAddr(addr eacutil.Address, minConf int32) (eacutil.Amount, error) {
	var amount eacutil.Amount
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) SendOutputs(outputs []*wire.TxOut, account uint32,
	minconf int32, satPerKb eacutil.Amount, label string) (*wire.MsgTx, error) {
...
func (w *Wallet) SignTransaction(tx *wire.MsgTx, hashType txscript.SigHashType,
	...
	additionalKeysByAddress map[string]*eacutil.WIF,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, txIn := range tx.TxIn {
			...
			getKey := txscript.KeyClosure(func(addr eacutil.Address) (*btcec.PrivateKey, bool, error) {
			...
			getScript := txscript.ScriptClosure(func(addr eacutil.Address) ([]byte, error) {
