...
func p2pkhOutputs(amounts ...eacutil.Amount) []*wire.TxOut {
...
func makeInputSource(unspents []*wire.TxOut) InputSource {
	...
	currentInputValues := make([]eacutil.Amount, 0, len(unspents))
	f := func(target eacutil.Amount) (eacutil.Amount, []*wire.TxIn, []eacutil.Amount, [][]byte, error) {
		for currentTotal < target && len(unspents) != 0 {
			u := unspents[0]
			unspents = unspents[1:]
			nextInput := wire.NewTxIn(&wire.OutPoint{}, nil, nil)
			currentTotal += eacutil.Amount(u.Value)
			currentInputs = append(currentInputs, nextInput)
			currentInputValues = append(currentInputValues, eacutil.Amount(u.Value))
		}
...
func TestNewUnsignedTransaction(t *testing.T) {
	tests := []struct {
		...
		Outputs          []*wire.TxOut
		RelayFee         eacutil.Amount
		ChangeAmount     eacutil.Amount
