...
import (
	...
	"github.com/eacsuite/eacwallet/wallet/txsizes"
)

// SumOutputValues sums up the list of TxOuts and returns an Amount.
func SumOutputValues(outputs []*wire.TxOut) (totalOutput eacutil.Amount) {
	for _, txOut := range outputs {
		totalOutput += eacutil.Amount(txOut.Value)
	}
	return totalOutput
}
...
// InputSourceError.
type InputSource func(target eacutil.Amount) (total eacutil.Amount, inputs []*wire.TxIn,
...
type AuthoredTx struct {
	Tx              *wire.MsgTx
	PrevScripts     [][]byte
	PrevInputValues []eacutil.Amount
	TotalInput      eacutil.Amount
...
func NewUnsignedTransaction(outputs []*wire.TxOut, feeRatePerKb eacutil.Amount,
...
func AddAllInputScripts(tx *wire.MsgTx, prevPkScripts [][]byte, inputValues []eacutil.Amount,
...
func spendWitnessKeyHash(txIn *wire.TxIn, pkScript []byte,
	...
	p2wkhAddr, err := eacutil.NewAddressWitnessPubKeyHash(pubKeyHash, chainParams)
	if err != nil {
		return err
	}
