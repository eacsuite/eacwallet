...
const DefaultRelayFeePerKb eacutil.Amount = 1e3
...
func GetDustThreshold(scriptSize int, relayFeePerKb eacutil.Amount) eacutil.Amount {
	...
	relayFee := eacutil.Amount(totalSize) * byteFee
	return 3 * relayFee
}
...
func IsDustAmount(amount eacutil.Amount, scriptSize int, relayFeePerKb eacutil.Amount) bool {
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb eacutil.Amount) bool {
	...
	return IsDustAmount(eacutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
// output.
func CheckOutput(output *wire.TxOut, relayFeePerKb eacutil.Amount) error {
	if output.Value < 0 {
		return ErrAmountNegative
	}
	if output.Value > eacutil.MaxSatoshi {
...
func FeeForSerializeSize(relayFeePerKb eacutil.Amount, txSerializeSize int) eacutil.Amount {
	fee := relayFeePerKb * eacutil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > eacutil.MaxSatoshi {
		fee = eacutil.MaxSatoshi
	}
