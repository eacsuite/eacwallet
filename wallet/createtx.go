...
func makeInputSource(eligible []wtxmgr.Credit) txauthor.InputSource {
	...
	currentTotal := eacutil.Amount(0)
	currentInputs := make([]*wire.TxIn, 0, len(eligible))
	currentScripts := make([][]byte, 0, len(eligible))
	currentInputValues := make([]eacutil.Amount, 0, len(eligible))

	return func(target eacutil.Amount) (eacutil.Amount, []*wire.TxIn,
		[]eacutil.Amount, [][]byte, error) {
...
func (s secretSource) GetKey(addr eacutil.Address) (*btcec.PrivateKey, bool, error) {
...
func (s secretSource) GetScript(addr eacutil.Address) ([]byte, error) {
	ma, err := s.Address(s.addrmgrNs, addr)
	if err != nil {
		return nil, err
	}
...
func (w *Wallet) txToOutputs(outputs []*wire.TxOut, account uint32,
	minconf int32, feeSatPerKb eacutil.Amount, dryRun bool) (
	...
	changeSource := func() ([]byte, error) {
		...
		var changeAddr eacutil.Address
		var err error
	...
	if tx.ChangeIndex >= 0 && account == waddrmgr.ImportedAddrAccount {
		changeAmount := eacutil.Amount(tx.Tx.TxOut[tx.ChangeIndex].Value)
...
func validateMsgTx(tx *wire.MsgTx, prevScripts [][]byte, inputValues []eacutil.Amount) error {
