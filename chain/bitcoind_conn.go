...
const (
	...
	// notifications from earthcoind through ZMQ.
	...
	// from earthcoind through ZMQ.
	maxRawBlockSize = 4e6

	// maxRawTxSize is the maximum size in bytes for a raw transaction
	// received from earthcoind through ZMQ.
	maxRawTxSize = maxRawBlockSize

	// seqNumLen is the length of the sequence number of a message sent from
	// earthcoind through ZMQ.
	seqNumLen = 4
)

// EarthcoindConn represents a persistent client connection to a earthcoind node
...
type BitcoindConn struct {
	...
	// each new earthcoind rescan client using the current earthcoind
	// connection.
	rescanClientCounter uint64

	// chainParams identifies the current network the earthcoind node is
	// running on.
	chainParams *chaincfg.Params

	// client is the RPC client to the earthcoind node.
	...
	// rescanClients is the set of active earthcoind rescan clients to which
...
// Start attempts to establish a RPC and ZMQ connection to a earthcoind node. If
...
// Stop terminates the RPC and ZMQ connection to a earthcoind node and removes any
...
func (c *BitcoindConn) blockEventHandler() {
	...
	log.Info("Started listening for earthcoind block notifications via ZMQ "+
		"on", c.zmqBlockConn.RemoteAddr())

	// Set up the buffers we expect our messages to consume. ZMQ
	// messages from earthcoind include three parts: the command, the
	// data, and the sequence number.
	//
	// We'll allocate a fixed data slice that we'll reuse when reading
	// blocks from earthcoind through ZMQ. There's no need to recycle this
	...
	for {
		...
		default:
			...
			// earthcoind shuts down, which will produce an unreadable
...
func (c *BitcoindConn) txEventHandler() {
	...
	log.Info("Started listening for earthcoind transaction notifications "+
	...
	//
	// We'll allocate a fixed data slice that we'll reuse when reading
	// transactions from earthcoind through ZMQ. There's no need to recycle
...
// getCurrentNet returns the network on which the earthcoind node is running.
...
// NewBitcoindClient returns a earthcoind client using the current earthcoind
