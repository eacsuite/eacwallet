...
// NeutrinoClient is an implementation of the eacwallet chain.Interface interface.
...
func (s *NeutrinoClient) GetBlock(hash *chainhash.Hash) (*wire.MsgBlock, error) {
	...
	//  Should the block cache be INSIDE neutrino instead of in eacwallet?
...
func (s *NeutrinoClient) Rescan(startHash *chainhash.Hash, addrs []eacutil.Address,
	outPoints map[wire.OutPoint]eacutil.Address) error {
...
func (s *NeutrinoClient) NotifyBlocks() error {
	...
	if !s.scanning {
		s.clientMtx.Unlock()
		return s.NotifyReceived([]eacutil.Address{})
	}
	s.clientMtx.Unlock()
	return nil
}
...
func (s *NeutrinoClient) NotifyReceived(addrs []eacutil.Address) error {
	s.clientMtx.Lock()
...
func (s *NeutrinoClient) onFilteredBlockConnected(height int32,
	header *wire.BlockHeader, relevantTxs []*eacutil.Tx) {
